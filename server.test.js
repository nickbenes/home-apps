...
describe('Todo API', () => {
  describe('GET /', () => {
    it('should return API info', async () => {
      ...
      expect(res.text).toContain('Todo API');
    });
  });
  ...
  describe('Integration - Full CRUD flow', () => {
    it('should handle complete todo lifecycle', async () => {
      ...
      // Read all
      const getAllRes = await request(app).get('/todos');
      expect(getAllRes.statusCode).toBe(200);
      expect(getAllRes.body.some(t => t.id === todoId)).toBe(true);

      // Read one
      const getOneRes = await request(app).get(`/todos/${todoId}`);
      ...
      // Verify deleted
      const verifyRes = await request(app).get(`/todos/${todoId}`);
